...
if( file_exists( $fn ) ) {
	...
	$this->SetTemplate(TMPL_PREFIX_DISPLAY.'Sample',$template);
	
	//Default values
	$this->SetPreference('default_'.TMPL_PREFIX_DISPLAY.'_template_contents',$template);
	$this->SetPreference(DEFAULT_DISPLAY_TMPL_PREF_NAME,'Sample');
}
# Setup 2nd display template
$fn = dirname(__FILE__).'/templates/sample_with_pretty_urls.tpl';
if( file_exists( $fn ) ) {
	$template = @file_get_contents($fn);
	$this->SetTemplate(TMPL_PREFIX_DISPLAY.'Sample_With_Pretty_Url',$template);
}
